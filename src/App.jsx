...
const BuyCoinsPage = ({ db, userId, setPage, showNotification, globalConfig, userProfile }) => {
    ...
    const MERCHANT_NAME = "KH Market"; // ឈ្មោះបង្ហាញពេលស្កេន
    ...
    // មុខងារបញ្ជូនសំណើបង់ប្រាក់ (Manual Submit)
    const handleSubmitPayment = async () => {
        const cleanTrxId = trxId.trim();
        if(!cleanTrxId) return showNotification('សូមបញ្ចូលលេខប្រតិបត្តិការ (Trx ID)', 'error');

        setIsSubmitting(true);
        try {
            // 1. ឆែកមើលក្រែងលោលេខប្រតិបត្តិការនេះធ្លាប់ប្រើហើយ (Check Duplicate)
             const q = query(collection(db, 'artifacts', appId, 'public', 'data', 'deposits'), where('transactionId', '==', cleanTrxId));
             const snapshot = await getDocs(q);
             
             if(!snapshot.empty) {
                 showNotification('លេខប្រតិបត្តិការនេះត្រូវបានប្រើរួចហើយ!', 'error');
                 setIsSubmitting(false);
                 return;
             }

             // 2. បង្កើតសំណើដាក់លុយ (Status: pending)
             ...
             await setDoc(depositRef, { 
                 ...
                 status: 'pending', // រង់ចាំ Admin ត្រួតពិនិត្យ
                 date: serverTimestamp(), 
                 method: 'KHQR_MANUAL'
             });
             
             showNotification('បានបញ្ជូនសំណើជោគជ័យ! សូមរង់ចាំការត្រួតពិនិត្យ។', 'success'); 
             setSelectedPkg(null);
             setTrxId('');
    ...
    return (
        <div className="min-h-screen bg-purple-900 pb-16 pt-20 relative">
            ...
            {/* Popup សម្រាប់ស្កេន និងបញ្ចូលលេខប្រតិបត្តិការ */}
            {selectedPkg && (
                <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in zoom-in duration-200">
                    <div className="bg-white rounded-xl w-full max-w-sm p-5 text-center relative">
                        ...
                        {/* ប៊ូតុងបញ្ជូនសំណើ */}
                        <button onClick={handleSubmitPayment} disabled={isSubmitting} className={`w-full mt-5 ${isSubmitting ? 'bg-gray-400' : 'bg-blue-600 hover:bg-blue-700'} text-white font-bold py-3 rounded-xl shadow-lg text-sm flex justify-center items-center transition`}>
                             {isSubmitting ? <RefreshCw className="animate-spin w-4 h-4 mr-2"/> : <Zap className="w-4 h-4 mr-2"/>}
                             {isSubmitting ? 'កំពុងដំណើរការ...' : 'បញ្ជូនការបង់ប្រាក់ (Submit)'}
                        </button>
                    </div>
                </div>
            )}
