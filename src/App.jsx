...
const InfoMessage = ({ title, message, color = 'bg-yellow-100 border-yellow-500 text-yellow-700' }) => (
    <div className={`border-l-4 p-3 rounded-md shadow-sm ${color}`}>
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
            <Card className="w-full max-w-sm p-6 text-center">
                <Info className="w-8 h-8 mx-auto text-red-500 mb-3" />
                <h3 className="text-xl font-bold text-gray-800 mb-2">{title}</h3>
                <p className="mb-6 text-gray-600">{message}</p>
                <div className="flex justify-center space-x-3">
                    <button onClick={onCancel} className="py-2 px-4 rounded-lg text-gray-600 border border-gray-300 hover:bg-gray-100 transition">បោះបង់</button>
                    <button onClick={onConfirm} className="py-2 px-4 rounded-lg bg-red-500 text-white font-semibold hover:bg-red-600 transition">បញ្ជាក់</button>
                </div>
            </Card>
        </div>
    );
...
const EarnPage = ({ db, userId, type, setPage, showNotification, globalConfig }) => {
    ...
    const nextCampaign = () => {
        const next = campaigns.find(c => c.id !== current?.id);
                    ...
                    <Card className="p-4 text-center">
                        {type === 'view' ? (
                            <div className="aspect-video bg-black mb-4">
                                <iframe src={`https://www.youtube.com/embed/${current.link.split('v=')[1]?.split('&')[0]}?autoplay=0`} className="w-full h-full" frameBorder="0" allowFullScreen />
                            </div>
                        ) : (
    ...
    const handleRegister = async (email, password) => {
        try {
            ...
            await setDoc(getProfileDocRef(uid), {
                userId: uid, email, userName: `User_${shortId}`,
                points: 5000, shortId, createdAt: serverTimestamp(),
                referredBy: null
            });
